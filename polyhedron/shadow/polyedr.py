...
class Polyedr:
    ...
    # Сумма длин рёбер, оба конца которых — «хорошие» точки
    def good_edges_sum(self):
        s = 0.0
        for e in self.edges:
            # Проекция точки: p.x, p.y (z отбрасывается)
            xb, xf = e.beg.x, e.fin.x
            if (-1.0 < xb < 3.0) and (-1.0 < xf < 3.0):
                # Длина ребра в трёхмерном пространстве
                d = e.fin - e.beg
                s += (d.x**2 + d.y**2 + d.z**2) ** 0.5
        return s